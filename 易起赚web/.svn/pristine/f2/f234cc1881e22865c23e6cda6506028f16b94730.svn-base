    ...
    <div class="systemSet">
        ...
        <div class="infor-mod">
            <div class="add-mod-one">
                ...
                <div class="add-box">
                    <span class="add-tip"><i style="color: red">*</i> &nbsp;&nbsp;入库类型：</span>
                    <el-select class="add-ipt" clearable
                               :disabled="edit=='2'"
                               v-model="contentForm.warehouseOrderType" @change="selectWarehouseOrderType">
                        <el-option
                ...
                <div class="add-box">
                    <span class="add-tip"><i style="color: red">*</i> &nbsp;&nbsp;入库日期：</span>
                    <el-date-picker
                            style="width:60%"
                ...
                <h3 class="clearFox"><em></em>入库明细:</h3>
                <div>
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <el-input size="small"
                                          ...
                                          @input="saveTable(scope.row)" v-model="scope.row.batchNo"></el-input>
                            </template>
                        </el-table-column>
                        <el-table-column
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <el-input size="small"  type="text"
                                          maxlength="8"
                                          oninput="value=value.replace(/[^\d]/g,'')"
                                          @input="saveTable(scope.row)" v-model="scope.row.inWarehouseNumber"></el-input>
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="unit"
    ...
    export default {
        ...
        methods: {
            ...
            getQueryInWarehouseType(){
                MyPost('/banyan-inventory/warehouse/in/queryInWarehouseType', {
                    ...
                    if (code == '0') {
                        ...
                        if (this.edit=="2"){
                            for(let i=0;i<this.warehouseTypeList.length;i++){
                                if (this.contentForm.warehouseOrderType==this.warehouseTypeList[i].dictionaryKey&&this.warehouseTypeList[i].dictionaryValue=="其他入库"){
                                    this.warehouseMethod="2";
                                    this.warehouseBoolean=false;
                                    return
                                }else {
            ...
            getDetails(purchaseNo){
                MyPost('/banyan-procurement/purchaseReceive/order/queryDetailsByPurchaseReceiveNo', {
            ...
            getPurchaseNo(){
                MyPost('/banyan-procurement/purchaseReceive/order/queryPurchaseReceiveNo', {
                }).then((res) => {
                    let {code, message, data} = res;
                    if (code == '0') {
                        this.restaurants=[];
            ...
            saveAll() {
                ...
                if (!this.contentForm.businessNo&&this.warehouseMethod=="1"){
                ...
                for (var i = 0; i < this.tableData.length; i++) {
                    ...
                    if (this.checkNull(this.tableData[i].inWarehouseNumber)) {
                        this.$confirm('SKU编号:' + this.tableData[i].sku + ' 没有输入入库数量，请输入', '提示', {
                            confirmButtonText: '确定',
                            showCancelButton: false,
                            type: 'error'
                        }).then(() => {
                ...
                all = {
                    ...
                    warehouseOrderDetailsList: this.tableData
                };
                if (this.edit=="1"){
                    MyPostJson('/banyan-inventory/warehouse/in/save', all).then((res) => {
                    ...
                    MyPostJson('/banyan-inventory/warehouse/in/update', all).then((res) => {
                        this.loadingShow=false;
                        let {code, message, data} = res;
                        if (code == '0') {
                            this.$message({
            ...
            saveTable(item){
                ...
                if (item.inWarehouseNumber==null){
                    item.inWarehouseNumber=0;
                }else {
                    item.inWarehouseNumber=(item.inWarehouseNumber+"").replace(/[^\d]/g,'');
                }
                if(Number(item.inWarehouseNumber)>Number(item.shipNumber)&&this.warehouseMethod=="1"){
                    this.$confirm("入库数量不能超过收货数量", '提示', {
                        confirmButtonText: '确定',
                        showCancelButton: false,
                        type: 'error'
                    }).then(() => {
                    ...
                    item.inWarehouseNumber=item.shipNumber
                    return
                }
                item.totalPrice=Number(item.unitPrice)*Number(item.inWarehouseNumber);
                item.totalPrice=this.regexNum(item.totalPrice);
            },
            addBrandClk() {
