        ...
        <div class="contain-mod">
            ...
            <div class="contain-top">
                ...
                <el-input maxlength="50" @keyup.enter.native="queryListFnMall" size="mini" class="w106 mr30"
                          v-model="searchList.content"></el-input>
                <span class="search-btn">级别：</span>
                <el-select class="w120 mr30" size="mini" placeholder="级别" v-model="searchList.levelId">
                    <el-option v-for="item in levelItems"
                            :label="item.title"
                            :value="item.id">
                    </el-option>
                </el-select>
                <span class="search-btn">分类：</span>
                <el-select class="w120 mr30" size="mini" placeholder="分类" v-model="searchList.categoryId">
                    <el-option
                            label="全部"
                            value="">
                    </el-option>
                  ...
                  <el-table
                    ...
                    <el-table-column
                          ...
                          <template slot-scope="scope">
                              ...
                              <span class="scope_btn color-blue"
                                    @click="editClk(scope)">显示/隐藏</span>
							  <span class="scope_btn color-blue"
									      @click="editClk(scope)">修改</span>
							  <span class="scope_btn"
							        @click="delClkAll(scope)">删除</span>
                          </template>
                      </el-table-column>
                  </el-table>
                </div>
...
<script>
    ...
    export default {
        ...
        created() {
			...
			this.searchList = this.copyFrom(this.emptySearchList);
            this.queryListFn();
			this.loadLevelItems();
        },
        methods: {
			...
			// 获取级别列表
			loadLevelItems() {
				MyGet('/school/level/getSimpleList', {}).then((res) => {
            ...
            queryListFn() {
				this.tableData = [];
				var queryParam = this.copyFrom(this.searchList);
				queryParam.start = (this.currentPage - 1) * 10;
				queryParam.length = 10;
				console.info(queryParam);
                MyGet('/school/video/getItemList', queryParam).then((res) => {
                    let {code, message, data} = res;
					console.info(data);
                    if (code == '0') {
                        this.tableData = data.data;
                        for (let i = 0; i < this.tableData.length; i++) {
                            this.formatRow(this.tableData[i]);
                        }
			...
			formatRow(row) {
				if (row.createTime) {
					row.createTimeName = this.timeFormatYYMMDD(row.createTime);
				}
				row.visible = row.visible.toString().toLowerCase();
				row.visibleName = row.visible == 'true' ? '显示' : '隐藏';
			},
